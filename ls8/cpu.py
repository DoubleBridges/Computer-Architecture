...
class CPU:
    ...
    def alu(self, op, reg_a, reg_b):
        ...
        if op == self.opcodes["ADD"]:
            ...
            # If values in reg_a and reg_b are equal, set the E flag to 1 or 0 otherwise
            if self.reg[reg_a] == self.reg[reg_b]:
                self.fl = self.fl | 0b00000001

            # If value in reg_a is greater than value in reg_b, set the G flag to 1 or 0 otherwise
            if self.reg[reg_a] > self.reg[reg_b]:
                self.fl = self.fl | 0b00000010

            # If value in reg_a is less than value in reg_b, set the L flag to 1 or 0 otherwise
            if self.reg[reg_a] < self.reg[reg_b]:
    ...
    def trace(self):
        ...
        print(
            f"TRACE: PC: %02X | FL:  %02X | %02X %02X %02X |"
            % (
                self.pc,
                self.fl,
                # self.ie,
